...
**Scopes:**
The Candidate model provides several ActiveRecord scopes for advanced querying and filtering:

- `by_name(query)`: Filters candidates by first name, last name, or email (case-insensitive search)
  - Parameter: `query` (string) - Search term
  - Returns: Candidates matching the search term in name or email fields
  - Example: `Candidate.by_name("John")`

- `with_education`: Returns candidates who have at least one education record
  - No parameters
  - Returns: Distinct candidates with education records
  - Example: `Candidate.with_education`

- `experienced`: Returns candidates with recent work experience
  - No parameters
  - Returns: Candidates with work experience that is either ongoing (end_date IS NULL) or ended within the last year (end_date >= 1.year.ago)
  - Returns distinct candidates
  - Example: `Candidate.experienced`

- `by_skill(skill)`: Returns candidates whose work experience descriptions contain a specified skill
  - Parameter: `skill` (string) - Skill to search for (case-insensitive)
  - Returns: Distinct candidates with matching skills in work experience descriptions
  - Uses PostgreSQL `ILIKE` for case-insensitive pattern matching
  - Example: `Candidate.by_skill("Ruby")`

- `recent_applications`: Returns candidates with applications made in the last 30 days
  - No parameters
  - Returns: Distinct candidates with at least one application where application_date >= 30.days.ago
  - Example: `Candidate.recent_applications`

**Scope Combinability:**
All scopes can be chained together for complex queries:
```ruby
# Find experienced candidates with education who know Ruby and have recent applications
Candidate.with_education.experienced.by_skill("Ruby").recent_applications

# Combine with existing by_name scope
Candidate.by_name("John").with_education.experienced
```
